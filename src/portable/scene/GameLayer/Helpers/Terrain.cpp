...
void Terrain::initStreet()
{
    ...
    char filename[40] {0};
    if(m_selectedLevel == Level::kLevel_1 || m_selectedLevel == Level::kLevel_2)
    {        
    ...
    CCSprite *street = nullptr;
    CCSprite *patch = nullptr;
...
void Terrain::initObstacles()
{
    ...
    else
    {
        ...
        obstacle->setKind(Body::Kind::kAceite);
        obstacle->setVisible(false);
        m_obstacles->addObject(obstacle);
        ...
        if (m_selectedLevel == Level::kLevel_5)
        {
            obstacle = Obstacle::create("GamePlay/Gomas.png");
            obstacle->setKind(Body::Kind::kGomas);
            obstacle->setVisible(false);
    ...
    CCARRAY_FOREACH(m_obstacles, it)
    {
        m_mapObstaclesPlayerCollitions.insert(
                    std::pair<Collition, bool>({m_player, it}, false));
    }
}
...
void Terrain::initCars()
{
    ...
    else if (m_selectedLevel == Level::kLevel_2)
    {
        Car *car = nullptr;
    ...
    else if (m_selectedLevel == Level::kLevel_3)
    {
        Car *car = nullptr;
    ...
    else if (m_selectedLevel == Level::kLevel_4)
    {
        Car *car = nullptr;
    ...
    else if (m_selectedLevel == Level::kLevel_5)
    {
        Car *car = nullptr;
...
const Car::Footpath Terrain::getNextFootPath(const Body::Footpath &currentFP) const
{
    switch (currentFP)
    {
    case Car::Footpath::kLeft:
        return Car::Footpath::kCenter;
        break;
    case Car::Footpath::kCenter:
        return Car::Footpath::kRigth;
        break;
    case Car::Footpath::kRigth:
        return Car::Footpath::kLeft;
        break;
    default:
        return Car::Footpath::kCenter;
        break;
...
void Terrain::putObstacle ()
{
    if (m_obstaclesIdle > 0)
    {
        ...
        {
            obstacle->setFootpath(Body::Footpath::kRigth);
                obstacle->setPositionX(obstacle->getPositionX() +
                                       obstacle->getContentSize().width*.2f);
        }
        else
        {
            obstacle->setFootpath(Body::Footpath::kLeft);
                obstacle->setPositionX(obstacle->getPositionX() -
                                       obstacle->getContentSize().width*.2f);
        }
...
void Terrain::obstacleDone(CCObject *pSender)
{
    ...
    {
    case Body::Kind::kTree:
        ++m_treesIdle;
        static_cast<Body*>(pSender)->setVisible(false);
        static_cast<Body*>(pSender)->setActive(false);
        break;
    case Body::Kind::kGomas:
    case Body::Kind::kAceite:
    case Body::Kind::kBasurero:
        ++m_obstaclesIdle;
        static_cast<Body*>(pSender)->setVisible (false);
        static_cast<Body*>(pSender)->setActive(false);
        break;
...
void Terrain::changeFootpathTo (Car::Footpath changeTo)
{
...
{    
    ...
    CCARRAY_FOREACH (m_obstacles, it)
    {
        std::map<Collition, bool>::iterator index =
                m_mapObstaclesPlayerCollitions.find({m_player, it});
    ...
    {
        std::map<Collition, bool>::iterator index =
                m_mapCarsPlayerCollitions.find({m_player, it});
        if (index->second == true)
        {
            index->second = static_cast<Obstacle*>(it)->
                    getBaseForCollition().intersectsRect(playerRect);
    ...
    CCARRAY_FOREACH(m_cars, itCars)
    {
        ...
        {
            std::map<Collition, bool>::iterator index =
                    m_mapCarsObstaclesCollitions.find({itCars, itObstacles});
            if (index->second == true)
            {
