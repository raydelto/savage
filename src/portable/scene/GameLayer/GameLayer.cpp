...
GameLayer::GameLayer()
    ...
    , m_fail {nullptr}
    , m_menu {nullptr}
    , m_pMenuOnPause {nullptr}
    , m_animImpact {nullptr}
    , m_Exp0020 {nullptr}
{
}
...
void GameLayer::initMembers()
{
    ...
    this->addChild (m_timmerLabel, static_cast<int>(zOrder::zIndex_6));
    ...
    m_goToPlay->retain();
    this->addChild(m_goToPlay, static_cast<int>(zOrder::zIndex_6));
    ...
    m_wine->retain();
    m_wine->setScale(.1f);
    this->addChild(m_wine, static_cast<int>(zOrder::zIndex_6));
    ...
    m_fail->setPosition(CCPoint{m_visibleSize.width*.5f,
                            m_visibleSize.height*.5f});
    this->addChild(m_fail, static_cast<int>(zOrder::zIndex_6));

    CCObject *it = nullptr;
    const CCArray *str = m_terrain->getPatchs();
    CCARRAY_FOREACH(str, it)
    {
        this->addChild(static_cast<CCNode*>(it), static_cast<int>(zOrder::zIndex_0));
    }

    str = m_terrain->getStreet();
    CCARRAY_FOREACH(str, it)
    {
        this->addChild(static_cast<CCNode*>(it), static_cast<int>(zOrder::zIndex_1));
    }

    str = m_terrain->getTrees();
    CCARRAY_FOREACH(str, it)
    {
        this->addChild(static_cast<CCNode*>(it), static_cast<int>(zOrder::zIndex_5));
    }

    str = m_terrain->getObstacles();
    CCARRAY_FOREACH(str, it)
    {
        this->addChild(static_cast<CCNode*>(it), static_cast<int>(zOrder::zIndex_2));
    }

    str = m_terrain->getCars();
    CCARRAY_FOREACH(str, it)
    {
        this->addChild(static_cast<CCNode*>(it), static_cast<int>(zOrder::zIndex_3));
    }

    Player *ptrToPlayer = const_cast<Player*>(m_terrain->getPlayer());
    this->addChild(ptrToPlayer, static_cast<int>(zOrder::zIndex_3));
    ...
    CCSpriteFrame *frame = nullptr;//NOTE: ver en google style guide
    ...
    m_Exp0020->retain();
    this->addChild(m_Exp0020, static_cast<int>(zOrder::zIndex_5));
...
void GameLayer::initMenu()
{
    ...
    m_menu->retain();
    this->addChild(m_menu, static_cast<int>(zOrder::zIndex_5));
    ...
    this->addChild(m_pMenuOnPause, static_cast<int>(zOrder::zIndex_6));
}
...
void GameLayer::putCar(float dt)
{
    ...
    if (car != nullptr)
    {
        ...
        {
        case 0:
            car->setFootpath(Car::Footpath::kCenter);
            break;
        case 1:
            car->setFootpath(Car::Footpath::kLeft);
            break;
        case 2:
            car->setFootpath(Car::Footpath::kRigth);
            break;
        default:
            break;
        }
...
void GameLayer::update(float dt)
{
    ...
    switch (m_gameStatus)
    {
    case Terrain::GameStatus::kGameOver:
        ...
        break;
    case Terrain::GameStatus::kCollided:
        m_timmer = m_cTimerValue;
        break;
    default:
        break;
...
bool GameLayer::ccTouchBegan(CCTouch *touch, CCEvent *event)
{
    ...
    return m_gameStatus != Terrain::GameStatus::kGameOver &&
            m_menu->isVisible();
}
    ...
    if (m_movinTouch)
    {
        ...
        else
        {
            if (horizontalComponent > 0)
             {
                Car::Footpath fp = m_terrain->getPlayer()->getFootpath();
                if (fp == Car::Footpath::kCenter)
                {
                    m_terrain->changeFootpathTo (Car::Footpath::kLeft);
                }
                else if (fp == Car::Footpath::kRigth)
                {
             ...
             else
             {
                ...
                if (fp == Car::Footpath::kCenter)
                {
                    m_terrain->changeFootpathTo (Car::Footpath::kRigth);
                }
                else
                {
                    if (horizontalComponent > m_visibleSize.width*.5f)
